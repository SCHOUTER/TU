...
\begin{defbox}[RSA Schlüsselgenerierung]
\begin{enumerate}
\item Wähle große Primzahlen $p$, $q$ mit $p \neq q$
\item Berechne $n = pq$ und $\varphi(n) = (p-1)(q-1)$
\item Wähle $e \in \mathbb{Z}_{\varphi(n)}^\times$ mit $1 \neq e$ und $ggT(e, \varphi(n)) = 1$
\item Berechne $d$ als multiplikativ Inverses zu $e$ modulo $\varphi(n)$: $ed \equiv 1 \pmod{\varphi(n)}$
\end{enumerate}
Öffentlicher Schlüssel: $(e, n)$ \\
Privater Schlüssel: $(d, n)$ \\
$p$, $q$, $\varphi(n)$ sollten nach Generierung gelöscht werden.
\end{defbox}

\noindent\textbf{Ver- und Entschlüsselung:}
\begin{itemize}
...
\begin{itemize}
\item Wähle zyklische Gruppe $G = (G, \circ, e)$ mit Erzeuger $g$
\item Wähle $a \in \{2, \dots, \text{ord}(g)-1\}$, berechne $A = g^a$
\item Privater Schlüssel: $(G, g, a)$
\item Öffentlicher Schlüssel: $(G, g, A)$
\end{itemize}
\end{defbox}

\noindent\textbf{Ver- und Entschlüsselung:}
\begin{itemize}
\item Verschlüsselung (an Alice): Wähle zufällig $r$, berechne $R = g^r$, $K = A^r = g^{ar}$, $C = m \circ K$, sende $(R, C)$
\item Entschlüsselung (Alice): Berechne $K = R^a = g^{ra}$, bestimme $K^{-1}$, $m = C \circ K^{-1}$
\end{itemize}

\noindent\textbf{Sicherheit von ElGamal:}
\begin{itemize}
\item Basierend auf \defc{diskretem Logarithmusproblem} (DLP)
\item Durch Quantencomputer (Shor) brüchbar
\item \defc{Probabilistische Verschlüsselung} $\rightarrow$ semantisch sicher wenn DDH-Problem schwer
\item Aus Chiffrat $(c_1, c_2)$ lässt sich $(c_1, g c_2)$ für alle $g \in G$ erzeugen
\end{itemize}
