...
public class CodeGenerator extends AstNodeBaseVisitor<Instruction, Void> {
	...
	public Instruction visitMatrixLhsIdentifier(MatrixLhsIdentifier matrixLhsIdentifier, Void __) {
		...
		MatrixType type = ((MatrixType) decl.getType());
	...
	public Instruction visitRecordLhsIdentifier(RecordLhsIdentifier recordLhsIdentifier, Void __) {
		...
		RecordType type = (RecordType) decl.getType();
		int base = decl.getLocalBaseOffset();
		...
		assembler.loadAddress(Register.LB, base + offset).addName(name)
				.addComment("load addresss of " + name + " (" + base + "[LB] + " + offset + ")", false);
	...
	public Instruction visitCompoundStatement(CompoundStatement compoundStatement, Void __) {
		// DONE implement (task 9.3)
		int scopeStartOffset = assembler.getNextOffset();
		compoundStatement.statements.forEach(this::visit);

		assembler.resetNextOffset(scopeStartOffset);
		return null;
	}
	...
	public Instruction visitDivision(Division division, Void __) {
		// DONE implement (task 9.1)
		visitArithmeticOperator(division, true, true, true, Primitive.divI, Primitive.divF);
		return null;
	}
	...
	public Instruction visitCompare(Compare compare, Void __) {
		// DONE implement (task 9.1)
		Type opType = compare.leftOperand.getType();
		visit(compare.leftOperand);
		visit(compare.rightOperand);

		if (opType.equals(IntType.instance)) {
			assembler.emitIntegerComparison(compare.comparator);
		} else if (opType.equals(FloatType.instance)) {
			assembler.emitFloatComparison(compare.comparator);
		} else if (opType.equals(BoolType.instance)) {
			assembler.emitIntegerComparison(compare.comparator);
		} else {
			throw new InternalCompilerError("Comparison not supported for type: " + opType);
		}
		return null;
	}
	...
	public Instruction visitMatrixTranspose(MatrixTranspose matrixTranspose, Void __) {
		// DONE implement (task 9.2)

		visit(matrixTranspose.operand);

		int size = matrixTranspose.getType().wordSize;
		assembler.emitPush(size);

		MatrixType ogType = (MatrixType) matrixTranspose.operand.getType();
		int rows = ogType.rows;
		int cols = ogType.cols;

		assembler.emitMatrixTranspose();

		assembler.emitPop(size, size);
