...
public class CodeGenerator extends AstNodeBaseVisitor<Instruction, Void> {
	...
	/**
	 * A wrapper around the <code>AstNode.accept</code> method which also manages
	 * the context stack required for generating certain debug symbols.
	 *
	...
	public Instruction visitVariableAssignment(VariableAssignment variableAssignment, Void __) {
		// TODO implement (task 9.4)
		visit(variableAssignment.value);
		visit(variableAssignment.identifier);
		int wordSize = variableAssignment.value.getType().wordSize;
	...
	public Instruction visitLeftHandIdentifier(LeftHandIdentifier leftHandIdentifier, Void __) {
		// TODO implement (task 9.4)
		Declaration declaration = leftHandIdentifier.getDeclaration();
		assembler.loadAddress(Register.LB, declaration.getLocalBaseOffset()).addName(leftHandIdentifier.name)
				.addComment("laod address of " + leftHandIdentifier.name);
		return null;
	}
	...
	public Instruction visitMatrixLhsIdentifier(MatrixLhsIdentifier matrixLhsIdentifier, Void __) {
		// TODO implement (task 9.4)
		Declaration declaration = matrixLhsIdentifier.getDeclaration();
		MatrixType type = (MatrixType) declaration.getType();

		visitLeftHandIdentifier(matrixLhsIdentifier, null);
	...
	public Instruction visitVectorLhsIdentifier(VectorLhsIdentifier vectorLhsIdentifier, Void __) {
		// TODO implement (task 9.4)
		Declaration declaration = vectorLhsIdentifier.getDeclaration();
		VectorType type = (VectorType) declaration.getType();
		visitLeftHandIdentifier(vectorLhsIdentifier, null);
	...
	public Instruction visitRecordLhsIdentifier(RecordLhsIdentifier recordLhsIdentifier, Void __) {
		// TODO implement (task 9.4)
		Declaration declaration = recordLhsIdentifier.getDeclaration();
		RecordType type = (RecordType) declaration.getType();
		int base = declaration.getLocalBaseOffset();
	...
	public Instruction visitIfStatement(IfStatement ifStatement, Void __) {
		...
		if (emitElse)
			jumpOverElseBranch = assembler.emitJump(-1);
	...
	public Instruction visitCompoundStatement(CompoundStatement compoundStatement, Void __) {
		// TODO implement (task 9.3)
		int startOffset = assembler.getNextOffset();
		compoundStatement.statements.forEach(this::visit);
		int count = assembler.getNextOffset() - startOffset;
		if (count > 0) {
			assembler.emitPop(0, count);
		}
		assembler.setNextOffset(startOffset);
		return null;
	}
	...
	public Instruction visitDivision(Division division, Void __) {
		...
		visitArithmeticOperator(division, false, true, true, Primitive.divI, Primitive.divF);
		return null;
	}
