...
\begin{defbox}[Admissible Heuristics]
  A heuristic is \textbf{admissible} if it \textbf{never overestimates} the cost to reach a goal.
  Formally: $h(n) \le h^(n)$ for all nodes $n$, where $h^(n)$ is the true cost from $n$ to the goal.
  (e.g., straight-line distance $h_{SLD}$ is admissible for route-finding).
\end{defbox}
