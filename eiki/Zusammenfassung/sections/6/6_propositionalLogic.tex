...
\begin{enumerate}
   \item Eliminiere $\Leftrightarrow$.
   \item Eliminiere $\Rightarrow$ (ersetze $A \Rightarrow B$ durch $\neg A \lor B$).
   \item Verschiebe $\neg$ nach innen (De Morgan, doppelte Negation).
   \item Wende Distributivgesetz an ($\lor$ über $\land$).
\end{enumerate}
...
\begin{enumerate}
   ...
   \item Wandle alles in CNF um.
   \item Wende wiederholt die \defc{Resolutionsregel} an:
   ...
   \item Wenn die \textbf{leere Klausel} (Widerspruch) abgeleitet wird, ist $\alpha$ bewiesen.
\end{enumerate}
...
\begin{itemize}
   ...
   \item \textbf{Regel-Explosion:} Für jede Instanz muss eine eigene Regel geschrieben werden (z.B. $Breeze_{1,1} \Leftrightarrow \cdots$, $Breeze_{1,2} \Leftrightarrow \cdots $).
